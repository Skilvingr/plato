...
impl Home {
    pub fn new(
        ...
        let (mut visible_books, dirs) = context.library.list(&current_directory, None, false);
        ...
        let selected_library = context.settings.selected_library;
        let library_settings = &context.settings.libraries[selected_library];
        let library_name = library_settings.name.clone();
        ...
        shelf.update(
            &mut visible_books[index_lower..index_upper],
            hub,
            &mut RenderQueue::new(),
            context,
        );
    ...
    fn update_shelf(
        ...
        shelf.update(
            &mut self.visible_books[index_lower..index_upper],
            hub,
            rq,
            context,
        );
    ...
    fn empty_trash(&mut self, hub: &Hub, rq: &mut RenderQueue, context: &mut Context) {
        ...
        let notif = Notification::new(message, None, hub, rq, context);
    ...
    fn reseed(&mut self, hub: &Hub, rq: &mut RenderQueue, context: &mut Context) {
        ...
        if let Some(top_bar) = self.child_mut(0).downcast_mut::<TopBar>() {
            top_bar.reseed(None, None, &mut RenderQueue::new(), context);
        }
