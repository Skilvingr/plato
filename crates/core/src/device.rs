...
impl fmt::Display for Model {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            #[cfg(feature = "sim")]
            Model::KaesarSimulator => write!(f, "Kaesar Simulator"),
        }
    }
}
...
pub struct Device {
    ...
    pub dims: (u32, u32),
    pub dpi: u16,
    pub input_handler: input::handlers::InputHandler,
    pub battery_sysfs: &'static str,
}
...
impl Device {
    pub fn new(product: &str, model_number: &str) -> Device {
        let mut device = match product {
            "kraken" => Device {
                model: Model::Glo,
                proto: TouchProto::Single,
                dims: (758, 1024),
                dpi: 212,
                input_handler: input::handlers::multi_touch::handle_touch,
            ...
            "snow" => Device {
                ...
                proto: TouchProto::MultiSnow,
                dims: (1080, 1440),
                dpi: 265,
                input_handler: input::handlers::snow::handle_touch,
                battery_sysfs: "/sys/class/power_supply/mc13892_bat",
            },
            "nova" => Device {
                model: Model::ClaraHD,
            ...
            #[cfg(feature = "sim")]
            "kaesar_simulator" => Device {
                model: Model::KaesarSimulator,
                proto: TouchProto::MultiSim,
                dims: (600, 800),
                dpi: 167,
                input_handler: input::handlers::snow::handle_touch,
                battery_sysfs: "emulator_sysroot/battery_sysfs",
            },
            _ => Device {
                ...
                dims: (600, 800),
                dpi: 167,
                input_handler: input::handlers::single_touch::handle_touch,
                battery_sysfs: "/sys/class/power_supply/mc13892_bat",
            },
        };
        ...
        match product {
            "luna" | "cadmus" | "io" => {
                if fs::exists("/sys/class/power_supply/battery").is_ok_and(|x| x) {
                    device.battery_sysfs = "/sys/class/power_supply/battery";
                } else if fs::exists("/sys/class/power_supply/bd71827_bat").is_ok_and(|x| x) {
                    device.battery_sysfs = "/sys/class/power_supply/bd71827_bat";
                }
            }
            _ => {}
        }
...
pub static CURRENT_DEVICE: LazyLock<Device> = LazyLock::new(|| {
