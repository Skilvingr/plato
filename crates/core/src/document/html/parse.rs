    ...
    if let Some(index) = value.find(|c: char| c.is_ascii_alphabetic()) {
        value[..index]
            ...
            .and_then(|v| match &value[index..] {
                ...
                "cm" => Some(mm_to_px(10.0 * v, dpi)),
                "mm" => Some(mm_to_px(v, dpi)),
...
pub fn parse_color(value: &str) -> Option<Colour> {
    if value.starts_with('#') {
        ...
        .ok()?;
        let color = Colour::from_rgb(&[red, green, blue]);
        Some(color)
    } else {
        match value {
            "black" => Some(BLACK),
...
mod tests {
    ...
    fn test_parse_color() {
        ...
        assert_eq!(a, Some(Colour::Rgb(0, 0, 0)));
        assert_eq!(b, Some(Colour::Rgb(255, 0, 0)));
        assert_eq!(c, Some(Colour::Rgb(0, 255, 0)));
        assert_eq!(d, Some(Colour::Rgb(0, 0, 255)));
        assert_eq!(e, Some(Colour::Rgb(255, 255, 255)));
    }
}
