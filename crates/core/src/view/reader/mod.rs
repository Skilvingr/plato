...
use crate::geom::{Axis, CycleDir, DiagDir, Dir, LinearDir, POINT_ZERO, Region, halves};
...
use crate::metadata::{DEFAULT_CONTRAST_EXPONENT, DEFAULT_CONTRAST_GREY};
use crate::settings::{
    BottomRightGestureAction, DEFAULT_FONT_PATH, EastStripAction, FinishedAction,
    SouthEastCornerAction, SouthStripAction, WestStripAction, guess_frontlight,
};
use crate::settings::{
...
impl Reader {
    pub fn new(
        ...
        open(&path).and_then(|mut doc| {
            ...
            if font_family != DEFAULT_FONT_FAMILY {
                doc.set_font_family(font_family, &settings.reader.ext_fonts_path);
            }
            ...
            if let Some(ref mut r) = info.reader {
                ...
                if let Some(grey) = r.contrast_grey {
                    contrast.grey = grey;
                }
            } else {
    ...
    fn go_to_page(
        ...
        if let Some(location) = loc {
            ...
            self.view_port.page_offset = POINT_ZERO;
            self.selection = None;
    ...
    fn _directional_scroll(
        ...
        if delta == POINT_ZERO || self.cache.is_empty() {
            return;
        }
        ...
        let vprect = rect![POINT_ZERO, pt!(vpw, vph)] + next_page_offset + frame.min;
    ...
    fn directional_scroll(
        ...
        self._directional_scroll(delta, hub, rq, context, None);
    }

    fn directional_scroll_fast(
        &mut self,
        delta: Point,
        hub: &Hub,
        rq: &mut RenderQueue,
        ...
        self._directional_scroll(delta, hub, rq, context, Some(UpdateMode::Fast));
    }
    ...
    fn go_to_neighbor(
        ...
        match loc {
            ...
            _ => match dir {
                ...
                CycleDir::Previous => {
                    let notif =
                        Notification::new("No previous page.".to_string(), None, hub, rq, context);
                    self.children.push(Box::new(notif) as Box<dyn View>);
                }
            },
        }
    ...
    fn update_tool_bar(&mut self, rq: &mut RenderQueue, context: &mut Context) {
        if let Some(index) = locate::<ToolBar>(self) {
            ...
            if self.reflowable {
                ...
                tool_bar.update_contrast_grey_slider(self.contrast.grey, rq);
            }
    ...
    fn update(
        ...
        match self.view_port.zoom_mode {
            ...
            ZoomMode::Custom(_) => {
                ...
                let vpr = rect![POINT_ZERO, pt!(vpw, vph)] + self.view_port.page_offset + frame.min;
    ...
    fn toggle_font_family_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::FontFamilyMenu) {
            ...
            let mut families = family_names(DEFAULT_FONT_PATH)
                .map_err(|e| eprintln!("Can't get default family names: {:#}.", e))
                .unwrap_or_default();

            if fs::exists(&context.settings.reader.ext_fonts_path).is_ok_and(|x| x) {
                families.append(
                    &mut family_names(&context.settings.reader.ext_fonts_path)
                        .map_err(|e| eprintln!("Can't get external family names: {:#}.", e))
                        .unwrap_or_default(),
                );
            }
    ...
    fn toggle_contrast_grey_menu(
        ...
        if let Some(index) = locate_by_id(self, ViewId::ContrastGreyMenu) {
            if let Some(true) = enable {
                return;
            }
            ...
            let entries = (1..=6)
                .map(|x| {
                    ...
                    EntryKind::RadioButton(
                        format!("{:.1}", g),
                        EntryId::SetContrastGrey(x),
                        (g - self.contrast.grey).abs() < f32::EPSILON,
                    )
                })
                .collect();
            let contrast_grey_menu = Menu::new(
                rect,
                ViewId::ContrastGreyMenu,
                MenuKind::DropDown,
                entries,
                context,
            );
            rq.add(RenderData::new(
                contrast_grey_menu.id(),
                *contrast_grey_menu.rect(),
                UpdateMode::Gui,
            ));
            self.children
                .push(Box::new(contrast_grey_menu) as Box<dyn View>);
        }
    }
    ...
    fn set_font_family(
...

        {
            ...
            let font_path = if font_family == DEFAULT_FONT_FAMILY {
                ...
                &context.settings.reader.ext_fonts_path
            };
    ...
    fn set_contrast_grey(
        ...
        if let Some(ref mut r) = self.info.reader {
            r.contrast_grey = Some(grey);
        }
        self.contrast.grey = grey;
        self.update(None, hub, rq, context);
        self.update_tool_bar(rq, context);
    }
    ...
    fn reseed(&mut self, rq: &mut RenderQueue, context: &mut Context) {
        if let Some(index) = locate::<TopBar>(self) {
            if let Some(top_bar) = self.child_mut(index).downcast_mut::<TopBar>() {
                top_bar.reseed(None, None, rq, context);
            }
        }
    ...
    fn quit(&mut self, context: &mut Context) {
        ...
        if let Some(ref mut r) = self.info.reader {
            ...
            if (self.contrast.exponent - DEFAULT_CONTRAST_EXPONENT).abs() > f32::EPSILON {
                ...
                if (self.contrast.grey - DEFAULT_CONTRAST_GREY).abs() > f32::EPSILON {
                    r.contrast_grey = Some(self.contrast.grey);
                } else {
                    r.contrast_grey = None;
                }
            } else {
...
impl View for Reader {
    fn handle_event(
        ...
        match *evt {
            ...
            Event::Gesture(GestureEvent::Movement { start, end }) => {
                if let State::Selection(_) = self.state {
                    return true;
                }

                if let ZoomMode::Custom(_) = self.view_port.zoom_mode {
                    self.directional_scroll_fast(
                        pt!(start.x - end.x, start.y - end.y),
                        hub,
                        rq,
                        context,
                    );
            ...
            Event::Gesture(GestureEvent::MovementEnded {}) if self.state == State::Panning => {
                self.state = State::Idle;
                self.update(None, hub, rq, context);
                true
            }
                                ...
                                if let Some(path) = context.settings.external_urls_queue.as_ref() {
                                        ...
                                        OpenOptions::new().create(true).append(true).open(path)
                                    {
                                        if let Err(e) = writeln!(file, "{}", link.text) {
                                            ...
                                            let notif =
                                                Notification::new(message, None, hub, rq, context);
                                            self.children.push(Box::new(notif) as Box<dyn View>);
                                        }
                                    }
                                }
            ...
            Event::Slider(SliderId::ContrastGrey, grey, FingerStatus::Up) => {
                self.set_contrast_grey(grey, hub, rq, context);
                true
            }
            ...
            Event::ToggleNear(ViewId::ContrastGreyMenu, rect) => {
                self.toggle_contrast_grey_menu(rect, None, rq, context);
                true
            }
            ...
            Event::EndOfSearch => {
                ...
                if results_count == 0 {
                    let notif =
                        Notification::new("No search results.".to_string(), None, hub, rq, context);
                    self.children.push(Box::new(notif) as Box<dyn View>);
                    self.toggle_search_bar(true, hub, rq, context);
                    hub.send(Event::Focus(Some(ViewId::ReaderSearchInput))).ok();
                }
            ...
            Event::Select(EntryId::Save) => {
                ...
                let notif = Notification::new(msg, None, hub, rq, context);
                self.children.push(Box::new(notif) as Box<dyn View>);
                true
            }
            ...
            Event::Select(EntryId::SetContrastGrey(v)) => {
                let grey = ((1 << 8) - (1 << (8 - v))) as f32;
                self.set_contrast_grey(grey, hub, rq, context);
                true
            }
