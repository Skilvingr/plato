...
(resp. receive events from) *Kaesar*. An event is a JSON object with a required
...
When a directory is deselected, *Kaesar* will send the `SIGTERM` signal to all
