...
impl DjvuContext {
    fn handle_message(&self) {
        unsafe {
            ...
            if (*msg).any.tag == MessageTag::DDJVU_ERROR {
                let msg = (*msg).error;
...
impl DjvuOpener {
    pub fn new() -> Option<DjvuOpener> {
        unsafe {
            let name = CString::new("Kaesar").unwrap();
    ...
    pub fn open<P: AsRef<Path>>(&self, path: P) -> Option<DjvuDocument> {
        unsafe {
            ...
            while ddjvu_job_status(job) < JobStatus::DDJVU_JOB_OK {
                self.0.handle_message();
            }
            if ddjvu_job_status(job) >= JobStatus::DDJVU_JOB_FAILED {
                None
            } else {
                Some(DjvuDocument {
                    ctx: self.0.clone(),
    ...
    pub fn pixmap(&self, scale: f32, samples: usize) -> Option<Pixmap> {
        unsafe {
            ...
            let style = if samples == 1 {
                FormatStyle::DDJVU_FORMAT_GREY8
            } else {
                FormatStyle::DDJVU_FORMAT_RGB24
            };
