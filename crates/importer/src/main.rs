use getopts::Options;
use kaesar_core::anyhow::{format_err, Context, Error};
use kaesar_core::chrono::NaiveDateTime;
use kaesar_core::helpers::datetime_format;
use kaesar_core::library::Library;
use kaesar_core::metadata::{consolidate, rename_from_info};
use kaesar_core::metadata::{extract_metadata_from_document, extract_metadata_from_filename};
use kaesar_core::settings::{ImportSettings, LibraryMode};
use std::env;
use std::path::Path;
...
fn main() -> Result<(), Error> {
    ...
    if matches.opt_present("h") {
        println!("{}", opts.usage("Usage: kaesar-import -h|-I|-C|-EFSN [-k ALLOWED_KINDS] [-e METADATA_KINDS] [-a ADDED_DATETIME] [-m LIBRARY_MODE] LIBRARY_PATH"));
        return Ok(());
    }
