...
# Adapted from https://github.com/koreader/KoboUSBMS/blob/master/scripts/start-usbms.sh
...
# c.f., https://github.com/Skilvingr/kaesar/blob/master/scripts/usb-enable.sh
...
USB_PRODUCT_ID=${PRODUCT_ID:-"0x6666"}
...
#       Follow Kaesar's lead, and hard-code in both cases.
...
legacy_usb() {
	...
	if [ -e "${MODULES_PATH}/g_mass_storage.ko" ] ; then
		...
		# shellcheck disable=SC2086
		checked_insmod "${GADGETS_PATH}/g_file_storage.ko" file="${PARTITIONS}" stall=0 removable=1 ${PARAMS}
	fi
