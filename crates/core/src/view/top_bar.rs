...
impl TopBar {
    pub fn new(rect: Rectangle, root_event: Event, title: String, context: &mut Context) -> TopBar {
        ...
        let capacity = context.battery.capacity().map_or(0, |v| v[0]);
        let status = context
            .battery
            .status()
    ...
    pub fn update_battery_widget(
        &mut self,
        capacity: Option<u8>,
        status: Option<battery::Status>,
        rq: &mut RenderQueue,
        context: &mut Context,
    ) {
        if let Some(battery_widget) = self.children[3].downcast_mut::<Battery>() {
            battery_widget.update(capacity, status, rq, context);
        }
    }
    ...
    pub fn reseed(
        ...
        self.update_battery_widget(capacity, status, rq, context);
    }
}
