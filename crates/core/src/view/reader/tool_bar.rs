...
impl ToolBar {
    pub fn new(
        ...
        if reflowable {
            ...
            let grey_icon_rect = rect![
                rect.min.x + side + slider_width,
                rect.min.y,
                rect.min.x + 2 * side + slider_width,
                rect.min.y + side
            ];
            let grey_icon = Icon::new(
                "grey",
                grey_icon_rect,
                Event::ToggleNear(ViewId::ContrastGreyMenu, grey_icon_rect),
            );
            children.push(Box::new(grey_icon) as Box<dyn View>);

            let contrast_grey = reader_info
                .and_then(|r| r.contrast_grey)
                .unwrap_or(DEFAULT_CONTRAST_GREY);
            let slider = Slider::new(
                rect![
                    rect.min.x + 2 * side + slider_width,
                    rect.min.y,
    ...
    pub fn update_contrast_grey_slider(&mut self, grey: f32, rq: &mut RenderQueue) {
        let slider = self.children[3].as_mut().downcast_mut::<Slider>().unwrap();
        slider.update(grey, rq);
    }
}
...
impl View for ToolBar {
    ...
    fn resize(&mut self, rect: Rectangle, hub: &Hub, rq: &mut RenderQueue, context: &mut Context) {
        ...
        if self.reflowable {
            ...
            self.children[index].resize(grey_icon_rect, hub, rq, context);
            index += 1;
