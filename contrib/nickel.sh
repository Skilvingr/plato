...
# NOTE: LD_LIBRARY_PATH is the only late export from rcS we don't siphon in kaesar.sh, for obvious reasons ;).
...
if grep -q "^${WIFI_MODULE} " "/proc/modules"; then
    killall -q -TERM wifi-enable.sh ip.sh essid.sh
...
# We don't care about horribly legacy stuff, because if people switch between nickel and Kaesar in the first place, I assume they're using a decently recent enough FW version.
